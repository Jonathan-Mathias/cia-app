// CIA App + Dossier Motor route-to-basic smoke test via Chrome DevTools Protocol.
// Requires:
...
// 4. node tools/qa-dossier-motor-route-to-basic-cdp.mjs

const DEBUG_BASE = 'http://127.0.0.1:9223';
const TARGET_URL = `http://127.0.0.1:8787/index.html?smoke=route-to-basic&ts=${Date.now()}`;
...
const expression = `(async () => {
  ...
  NPS = null;
  LEAD = { _id: 'lead_test_route_to_basic', nome: 'Teste Route to Basic', email: 'teste@example.com', waDDI: '55', waNum: '11999999999', idade: '33-39' };
  ...
  const note = document.querySelector('.primary-cta-note');
  return {
    ctaId: cta ? cta.getAttribute('data-primary-cta') : null,
    ctaHref: cta ? cta.getAttribute('href') : null,
    ctaTarget: cta ? cta.getAttribute('target') : null,
    ctaNote: note ? note.textContent : null,
    expectedBaseUrl: APP_CONFIG.links.owned.whatsappGroup,
...
try {
  ...
  if (value.ctaId !== 'dossier_route_to_basic') {
    throw new Error(`Expected dossier_route_to_basic, got ${value.ctaId}`);
  }
  if (value.ctaHref !== value.expectedBaseUrl) {
    throw new Error(`Expected ${value.expectedBaseUrl}, got ${value.ctaHref}`);
  }
  if (!value.storedDecision || value.storedDecision.decision?.status !== 'Recusa') {
    throw new Error('Stored motor decision missing or invalid for Recusa');
  }
